...
default_to_fetch = [
    (1, 'obj', r'^best obj:\s(\d+\.?\d*)'),
    (3, 'ittot', r'^total iterations:\s(\d+\.?\d*)'),
    (2, 'itbest', r'^best iteration:\s(\d+\.?\d*)'),
    (5, 'ttot', r'^total time \[s\]:\s(\d+\.?\d*)'),
    (4, 'tbest', r'^best time \[s\]:\s(\d+\.?\d*)'),
    (100, 'obj0', r'^0+\s(\d+.?\d*)'),
    (101, 'obj1', r'^0+1\s(\d+.?\d*)'),
]
...
def main():
    to_fetch = default_to_fetch
    parser = argparse.ArgumentParser(description='Summarize results for multiple mhlib runs from their .out files.')
